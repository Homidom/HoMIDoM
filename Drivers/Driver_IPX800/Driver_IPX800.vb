...
#Region "Fonctions génériques"
    ...
    Public Sub Start() Implements HoMIDom.HoMIDom.IDriver.Start
        Try
            ...
            Try
                elmt = "version"
                _Version = valueconfig.version
                If _Version <> "" Then
                    WriteLog("Nom : " & valueconfig.config_hostname.ToString)
                    WriteLog("Version logicielle : " & _Version)
                    _IsConnect = True
        ...
        Try
            ...
            Select Case Objet.Type
                ...
                Case "GENERIQUEVALUE"
                    ...
                    Else
                        Objet.Value = GET_VALUE(_urlIPX & "globalstatus.xml", _Username, _Password, "analog" & Mid(Objet.Adresse1, InStr(Objet.Adresse1, ":") + 1, 1) - 1)
                    End If
                Case "GENERIQUESTRING"
                    If String.IsNullOrEmpty(Objet.adresse2) Then
                        Objet.Value = GET_VALUE(_urlIPX & "globalstatus.xml", _Username, _Password, Objet.Adresse1)
                    Else
                        'permet de reinitialier par ex un compteur
                        SEND_IPX800(_urlIPX & Objet.adresse2, _Username, _Password)
                        WriteLog("DBG: Write " & Objet.Type & " Adr : " & Objet.adresse1 & " -> Transmis")
                        Objet.Value = 1
                    End If
            End Select
    ...
    Public Sub New()
        Try
            ...
            'Libellé Device
            Add_LibelleDevice("ADRESSE1", "Numéro entrée/sortie", "Numéro entrée/sortie", "")
            Add_LibelleDevice("ADRESSE2", "Cmd. prise littéralement sans l'url de l'IPX", "")
...
#Region "Fonctions internes"
    ...
    Private Sub Get_Config(adrs As String, user As String, password As String)
        ...
        Try
            ...
            While reader.Read()
                WriteLog("DBG: " & reader.Name & " -> " & reader.ReadString)
                Select Case reader.Name
                    Case "config_hostname"
                        valueconfig.config_hostname = reader.ReadString
                    Case "version"
                        valueconfig.version = reader.ReadString
                    Case "http_port"
                        valueconfig.http_port = reader.ReadString
                    Case "dnsstatus"
                        valueconfig.dnsstatus = reader.ReadString
                    Case "day"
                        valueconfig.day = reader.ReadString
                    Case "time0"
                        valueconfig.time0 = reader.ReadString
                    Case "pingmsg"
                        valueconfig.pingmsg = reader.ReadString
                    Case "psel"
                        valueconfig.psel = reader.ReadString
                    Case "pingip"""
                        valueconfig.pingip = reader.ReadString
                    Case "pingretry"
                        valueconfig.pingretry = reader.ReadString
                    Case "pingtime"
                        valueconfig.pingtime = reader.ReadString
                End Select
            End While
        Catch ex As Exception
    ...
    Public Function GET_VALUE(adrs As String, user As String, password As String, ByVal Element As String) As Object
        ...
        Try
            ...
            While reader.Read()
                Select Case reader.Name
                    Case Element
                        result = reader.ReadString.ToString
                        WriteLog("DBG: " & "Valeur trouvée  pour " & Element & " -> " & result)
                        If IsNumeric(result) = False Then
                            If LCase(result.ToString) = "up" Then
                                result = 1
                            Else
                                If LCase(result.ToString) = "dn" Then
                                    result = 0
                                End If
                            End If
                        End If
