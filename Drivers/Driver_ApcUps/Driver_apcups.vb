...
Public Class Driver_apcups
    Implements HoMIDom.HoMIDom.IDriver

#Region "Variables génériques"
    '!!!Attention les variables ci-dessous doivent avoir une valeur par défaut obligatoirement
    'aller sur l'adresse http://www.somacon.com/p113.php pour avoir un ID
    Dim _ID As String = "1288616C-FB01-11E4-BE67-7C0C1E5D46B0"
    Dim _Nom As String = "ApcUps"
    Dim _Enable As Boolean = False
    Dim _Description As String = "Onduleur APC USB / COM"
    ...
    Dim productID As Integer = Convert.ToInt32("0x0002", 16)


#End Region

#Region "Variables Internes"
    Dim ups As HidDevice
...
#Region "Fonctions génériques"
    ...
    Public Sub Start() Implements HoMIDom.HoMIDom.IDriver.Start
        Try
            If _Modele = "APC UPS" Then vendorID = Convert.ToInt32("0x051D", 16)

            ' tente la connection USB
            If _COM = "USB" Then
                ups = HidDevices.Enumerate(vendorID, productID).FirstOrDefault
                '            ups = HidDevices.Enumerate(vendorId, productId).FirstOrDefault
                Application.DoEvents()

                If ups.IsConnected Then
                    WriteLog(ups.Description.ToUpper & " " & "VendorID = " & ups.Attributes.VendorId.ToString & " " & "ProductID = " & ups.Attributes.ProductId.ToString &
                             ups.DevicePath.ToString)
                    Application.DoEvents()
                    _IsConnect = True
                End If
            End If
        Catch ex As Exception
            _IsConnect = False
            WriteLog("ERR: ApcUps non démarré")
        End Try
    End Sub
    ...
    Public Sub New()
        ...
        Catch ex As Exception
            WriteLog("ERR: ApcUps " & ex.Message)
        End Try
    End Sub
#End Region
    ...
    Private Sub WriteLog(ByVal message As String)
        Try
            ...
            If STRGS.InStr(message, "DBG:") > 0 Then
                If _DEBUG Then
                    _Server.Log(TypeLog.DEBUG, TypeSource.DRIVER, "ApcUps", STRGS.Right(message, message.Length - 5))
                End If
            ElseIf STRGS.InStr(message, "ERR:") > 0 Then
                _Server.Log(TypeLog.ERREUR, TypeSource.DRIVER, "ApcUps", STRGS.Right(message, message.Length - 5))
            Else
                _Server.Log(TypeLog.INFO, TypeSource.DRIVER, "ApcUps", message)
            End If
        Catch ex As Exception
            _Server.Log(TypeLog.ERREUR, TypeSource.DRIVER, "ApcUps WriteLog", ex.Message)
        End Try
    End Sub
