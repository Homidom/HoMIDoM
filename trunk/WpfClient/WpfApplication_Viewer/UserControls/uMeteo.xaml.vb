...
Partial Public Class uMeteo
    ...
    Private Sub GetMeteo()
        Try
            ...
            ' Load data   
            doc.Load("http://weather.yahooapis.com/forecastrss?w=" & _Code & "&u=c")
            ...
            For Each node As XmlNode In nodes
                ...
                'Meteo pour Jour actuel
                If IDay = 0 Then
                    LblTemps.Content = Traduire(node.Attributes("text").InnerText)
                    Lbl0Min.Content = "Min: " & node.Attributes("low").InnerText & "°"
                    Lbl0Max.Content = "Max: " & node.Attributes("high").InnerText & "°"
                    If File.Exists("C:\ehome\images\meteo\" & node.Attributes("code").InnerText & ".png") = True Then
                        bmpImage.UriSource = New Uri("C:\ehome\images\meteo\" & node.Attributes("code").InnerText & ".png", UriKind.Absolute)
                    Else
                    ...
                    Day0.Content = TraduireJour(node.Attributes("day").InnerText)
                    MinD0.Content = node.Attributes("low").InnerText & "°"
                    MaxD0.Content = node.Attributes("high").InnerText & "°"
                    ImgD0.Source = bmpImage
                End If
                'Meteo pour jour suivant
                If IDay = 1 Then
                    If File.Exists("C:\ehome\images\meteo\" & node.Attributes("code").InnerText & ".png") = True Then
                        bmpImage.UriSource = New Uri("C:\ehome\images\meteo\" & node.Attributes("code").InnerText & ".png", UriKind.Absolute)
                    Else
                    ...
                    End If
                    bmpImage.EndInit()
                    Day1.Content = TraduireJour(node.Attributes("day").InnerText)
                    MinD1.Content = node.Attributes("low").InnerText & "°"
                    MaxD1.Content = node.Attributes("high").InnerText & "°"
                    ImgD1.Source = bmpImage
                End If
                IDay += 1
            Next

            nodes = doc.SelectNodes("/rss/channel/yweather:wind", ns)
            For Each node As XmlNode In nodes
                Dim a As String
                a = node.Attributes("chill").InnerText & " "
                LblTemp.Content = a & "°"
            Next
        Catch ex As Exception
            WriteLog("Erreur GetMeteo: " & ex.Message)
        End Try
    End Sub
